...
describe('Appointment Actions', () => {
  ...
  describe('getunavailableDates', () => {
    it('should return unavailable dates for a worker', async () => {
      ...
      const mockWorkerClosedDates = [
        {id: "id1", fecha: new Date('2023-06-15T12:00:00Z'), trabajadorId: trabajadorId, motivo: 'Vacation'},
      ];
      const mockGlobalClosedDates = [
        {id:"id-2", fecha: new Date('2023-06-20T12:00:00Z'), motivo: 'Holiday'} 
      ];

      prismaMock.diaCerradoTrabajador.findMany.mockResolvedValue(mockWorkerClosedDates);
      prismaMock.diaCerradoGlobal.findMany.mockResolvedValue(mockGlobalClosedDates);
    ...
    it('should handle errors and return error response', async () => {
      // Arrange
      const trabajadorId = 'worker-123';
      prismaMock.diaCerradoTrabajador.findMany.mockRejectedValue(new Error('DB Error'));
      ...
      expect(result.message).toContain('Error al obtener las fechas no disponibles');
    });
  });
  ...
  describe('getAvailableDates', () => {
    it('should return available times for a date', async () => {
      ...
      prismaMock.cita.findMany.mockResolvedValue([
        {
          id: 'cita-1',
          fechaHora: new Date(`${date}T09:00:00`),
      ...
      // Act
      const result = await getAvailableDates(date, trabajadorId, duracionMinutos);
    ...
    it('should handle errors and return error response', async () => {
      ...
      const date = '2023-06-16';
      const trabajadorId = 'worker-123';
      prismaMock.diaCerradoGlobal.findFirst.mockRejectedValue(new Error('DB Error'));
  ...
  describe('createBlockedDay', () => {
    it('should create blocked day successfully', async () => {
      ...
      prismaMock.diaCerradoGlobal.create.mockResolvedValue(mockCreatedBlockedDay);
